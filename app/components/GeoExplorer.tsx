...
const SURFACE_COPY:Record<SurfaceMode,{icon:string;title:string;body:string;source:string;swatches:string[]}>= {
  Natural:{icon:"◈",title:"Natural surface",body:"Full-color imagery with subtle relief that softens as you move closer.",source:"Esri imagery + Mapzen elevation",swatches:["#6f8060","#a48a67","#d3c4a2"]},
  "Bare Earth":{icon:"△",title:"Bare-earth elevation",body:"A hypsometric elevation material with terrain form emphasized above imagery.",source:"Mapzen elevation",swatches:["#486e68","#9a9868","#b67e60","#e2ded0"]},
...
export default function GeoExplorer(){
  ...
  useEffect(()=>{
    ...
    Promise.all([import("maplibre-gl"),import("maplibre-contour")]).then(([maplibre,contourModule])=>{
      ...
      try{
        ...
        ],terrain:{source:"terrain",exaggeration:1.15},sky:{"sky-color":"#8fb9c7","horizon-color":"#dce9e6","fog-color":"#d7e2de","fog-ground-blend":0.18,"horizon-fog-blend":0.72,"sky-horizon-blend":0.84,"atmosphere-blend":0.32}}});
        ...
        m.on("load",()=>setReady(true));m.on("move",()=>{const z=m.getZoom();setZoom(z);if(z<=GLOBE_ENTER_ZOOM)terrainRevealDone.current=false;setMode(current=>z<=GLOBE_ENTER_ZOOM?"GLOBE":z>=TERRAIN_ENTER_ZOOM?"TERRAIN":current);});
  ...
  function goToView(view:typeof VIEWS[number]){
    ...
    // One cancellable flight: interrupted presets must never restart an old destination.
    m.easeTo({center:view.point,zoom:view.zoom,pitch:Math.min(view.pitch,55),bearing:view.bearing,duration:1200});
    clearSelection();
  }
  function closeTerrain(){
    const m=map.current;if(!m||!ready)return;
    terrainRevealDone.current=true;m.stop();setSceneLoading(true);
    m.easeTo({zoom:Math.max(m.getZoom(),10.5),pitch:55,bearing:m.getBearing(),duration:900});
  }
