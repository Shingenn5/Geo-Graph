...
export default function GeoExplorer(){
  ...
  const [ready,setReady]=useState(false),[mode,setMode]=useState<ViewMode>("GLOBE"),[zoom,setZoom]=useState(1.65),[base,setBase]=useState<"satellite"|"topo">("satellite"),[geology,setGeology]=useState(false),[faults,setFaults]=useState(false),[borders,setBorders]=useState(true),[hillshade,setHillshade]=useState(true);
  ...
  useEffect(()=>{
    ...
    import("maplibre-gl").then(({Map,NavigationControl,ScaleControl,AttributionControl,setWorkerUrl})=>{
      ...
      try{
        ...
        const m=new Map({container:container.current,center:[-18,24],zoom:1.65,pitch:0,bearing:0,maxZoom:18,maxPitch:65,centerClampedToGround:true,aroundCenter:true,attributionControl:false,canvasContextAttributes:{antialias:true},style:{version:8,projection:{type:"globe"},sources:{
  ...
  const kpis=[
    {label:"SURVEY STATUS",value:soil?"Mapped unit":"Awaiting point",tone:soil?"good":"neutral"},
  ...
  const useCaseFacts:Record<UseCase,Array<[string,string]>>={
    ...
    Construction:[["Restriction",soil?.restrictionKind?`${soil.restrictionKind}${soil.restrictionDepthCm!=null?` · ${soil.restrictionDepthCm} cm`:""}`:"—"],["Concrete corrosion",soil?.concreteCorrosion||"—"],["Steel corrosion",soil?.steelCorrosion||"—"],["Frost action",soil?.frostAction||"—"],["Surface plasticity index",surface?.plasticityIndex!=null?String(surface.plasticityIndex):"—"]],
    Environment:[["Hydric condition",soil?.hydricCondition||soil?.hydricRating||"—"],["Surface CEC",surface?.cationExchangeCapacity!=null?`${surface.cationExchangeCapacity} cmol(+)/kg`:"—"],["Electrical conductivity",surface?.electricalConductivity!=null?`${surface.electricalConductivity} dS/m`:"—"],["Erosion K factor",surface?.erosionKFactor!=null?String(surface.erosionKFactor):"—"],["Rock fragments",surface?.rockFragmentsPercent!=null?`${surface.rockFragmentsPercent}%`:"—"]],
  };
  // eslint-disable-next-line @next/next/no-html-link-for-pages
  return <main className="app-shell"><header><a className="brand" href="/" aria-label="Geo Graph home"><span className="brand-icon">◈</span> GEO GRAPH</a><span className="header-note">Earth → terrain → survey</span><span className="view-readout"><b>{mode}</b><span>ZOOM {zoom.toFixed(1)}</span></span></header><div className="workspace"><aside>
    <section className="intro dashboard-intro"><div className="intro-heading"><div><p className="eyebrow">SOIL INTELLIGENCE</p><h1>Survey the ground with context.</h1></div><span className="status-dot" aria-label={ready?"Map ready":"Map loading"}/></div><p>Choose a point on the map to bring together USDA soil data, geology, and three-dimensional terrain.</p><div className="kpi-grid">{kpis.map(kpi=><div className={`kpi-card ${kpi.tone}`} key={kpi.label}><span>{kpi.label}</span><strong>{kpi.value}</strong></div>)}</div></section>
    ...
    <section className="coverage"><p>Soils: USDA NRCS Soil Data Access / SSURGO. Terrain: Mapzen and source contributors. Geology: Macrostrat and original survey authors. Coverage and detail vary.</p></section>
  </aside><div className="map-shell"><div ref={container} className="map" aria-label="Interactive 3D globe and terrain"/>{!ready&&!mapError&&<div className="map-message" role="status">Building the globe…</div>}{sceneLoading&&<div className="scene-loading" role="status">Loading elevation and imagery…</div>}{mapError&&<div className="map-message error" role="alert">{mapError}<button onClick={()=>setMapError("")}>×</button></div>}<div className="mode-chip"><span>{mode}</span><strong>{mode==="GLOBE"?"Rotate around the center":"Center the ground to inspect"}</strong></div><div className="camera-tools"><button className="primary-camera" onClick={selectCenter}>⌖ Select center</button>{point&&<button onClick={returnToSelection}>↩ Selection</button>}<button onClick={()=>goToView(VIEWS[0])}>◎ Globe</button><button onClick={closeTerrain}>◢ Terrain</button></div><div className="map-help">Move the map to position your location · Right-drag or two-finger drag to orbit · Select center to inspect</div></div></div></main>;
}
