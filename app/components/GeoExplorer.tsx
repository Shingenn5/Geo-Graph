...
export default function GeoExplorer(){
  ...
  function exportSurvey(format:"html"|"json"|"geojson"){
    ...
    const selection=createSurveySelection({
      ...
      context:[{label:"Terrain elevation (metres)",value:terrainElevation,source:"Mapterhorn DEM and source contributors",kind:"derived",method:"MapLibre terrain elevation sampled at the selected WGS84 point; native source resolution and vertical datum vary by tile."}],
    });
  ...
  return <main className="app-shell"><header><a className="brand" href="/" aria-label="Geo Graph home"><span className="brand-icon">◈</span><span>GEO GRAPH<small>Terrain intelligence</small></span></a><span className="header-note"><b>Survey workspace</b><small>Surface analysis console</small></span><span className="view-readout"><i className={ready?"online":""}/><b>{ready?"SYSTEM READY":"INITIALIZING"}</b><span>{mode} · Z{zoom.toFixed(1)}</span></span></header><div className="workspace"><aside>
    ...
    {(area||areaError)&&<section className="area-panel"><div className="section-heading"><div><p className="eyebrow">SELECTED AREA</p><h2>Ground footprint</h2></div>{area&&<button className="mini-export" onClick={clearSelection}>Clear</button>}</div>{areaError&&<p role="alert" className="error">{areaError} Choose two different corners.</p>}{area&&<><p><strong>{area.area.hectares.toLocaleString(undefined,{maximumFractionDigits:2})} hectares</strong> · {area.area.acres.toLocaleString(undefined,{maximumFractionDigits:2})} acres</p><p>WGS84 rectangle between {area.bbox.south.toFixed(5)}° and {area.bbox.north.toFixed(5)}° latitude. Area follows parallels and meridians on the WGS84 ellipsoid.</p><p>Soil and geology vary inside the boundary. This footprint does not claim a single subsurface condition.</p><button className="area-export" onClick={exportArea}>⇩ Export area GeoJSON</button></>}</section>}
